        ...
        <div class="bg-white p-6 rounded-xl shadow">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Klik edit untuk memperbarui layanan kurir atau hapus bila tidak lagi
                        aktif.</p>
                </div>
                <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-600">Total
                    {{ $couriers->count() }} layanan</span>
            </div>
                    ...
                    <tbody>
                        @forelse($couriers as $courier)
                            <tr class="border-b last:border-b-0 hover:bg-slate-50">
                                ...
                                <td class="p-4 align-top text-slate-700">Rp {{ number_format($courier->price, 0, ',', '.') }}
                                </td>
                                <td class="p-4 align-top">
                                    <span
                                        class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $courier->status === 'aktif' ? 'bg-emerald-100 text-emerald-800' : ($courier->status === 'nonaktif' ? 'bg-slate-100 text-slate-700' : 'bg-red-100 text-red-800') }}">{{ ucfirst($courier->status) }}</span>
                                </td>
                                <td class="p-4 flex flex-wrap gap-2">
                                    <!--<button type="button" data-courier='{{ e($courier->toJson()) }}'
                                                onclick="openEditCourier(JSON.parse(this.dataset.courier))"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-6 text-slate-500">Belum ada data kurir. Tambahkan layanan kurir
                                    terlebih dahulu.</td>
                            </tr>
                        @endforelse
                    </tbody>
